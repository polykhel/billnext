...
/**
 * Performance test for the WalletGroup entity.
 */
class WalletGroupGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the WalletGroup entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all walletGroups")
            .get("/api/wallet-groups")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new walletGroup")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_walletGroup_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created walletGroup")
                .get("${new_walletGroup_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created walletGroup")
            .delete("${new_walletGroup_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
